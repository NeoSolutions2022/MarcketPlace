...
public class FornecedorService : BaseService, IFornecedorService
{
    ...
    {
        var fornecedor = await _fornecedorRepository.FistOrDefault(f => f.Email == email);
        if (fornecedor == null)
        {
            Notificator.HandleNotFoundResource();
            return;
    ...
    {
        var cliente = await _fornecedorRepository.ObterPorId(id);
        if (cliente == null)
        {
            Notificator.HandleNotFoundResource();
            return;
        ...
        _fornecedorRepository.Remover(cliente);
        if (await _fornecedorRepository.UnitOfWork.Commit())
        {
            return;
        }
